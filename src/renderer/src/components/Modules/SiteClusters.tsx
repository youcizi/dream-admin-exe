        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
            <div className="p-8 border-b border-slate-50 flex items-center justify-between">
              ...
              <button className="text-[10px] font-black uppercase tracking-widest text-indigo-600">
                浏览全库
              </button>
            </div>
            <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                ...
                <div
                  key={i}
                  className="group relative rounded-[2rem] border border-slate-100 overflow-hidden cursor-pointer hover:border-indigo-200 transition-all"
                >
                  <div className="aspect-video bg-slate-50 relative overflow-hidden flex items-center justify-center">
                    ...
                    <div className="absolute inset-0 bg-indigo-900/0 group-hover:bg-indigo-900/40 transition-all flex items-center justify-center">
                      <button className="opacity-0 group-hover:opacity-100 px-6 py-2 bg-white text-indigo-900 rounded-full text-[10px] font-black uppercase tracking-widest transition-all translate-y-4 group-hover:translate-y-0">
                        预览模板
                      </button>
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white">
              ...
              <div className="space-y-6">
                {[
                  ...
                  <div key={item.step} className="flex gap-4 items-center">
                    ...
                    <span
                      className={`text-xs font-bold ${item.status === 'waiting' ? 'text-slate-600' : 'text-slate-200'}`}
                    >
                      {item.title}
                    </span>
                  </div>
              ...
              <button className="mt-8 w-full py-4 premium-gradient text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl active:scale-95 transition-all">
                立即开始 (AI Cluster Mode)
              </button>
            </div>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-slate-50">
                {[
                  ...
                  <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-8 py-6">
                      <span
                        className={`flex items-center gap-1.5 text-[10px] font-bold ${task.status === 'Completed' ? 'text-emerald-500' : 'text-indigo-500'}`}
                      >
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${task.status === 'Completed' ? 'bg-emerald-500' : 'bg-indigo-500 animate-pulse'}`}
                        />
                        {task.status}
                      </span>
                    </td>
          ...
          <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm relative overflow-hidden group">
            ...
            <div className="space-y-4">
              {[
                ...
                <div
                  key={i}
                  className="flex items-center justify-between p-4 border border-slate-50 rounded-2xl hover:bg-slate-50/50 transition-all"
                >
            ...
            <div className="relative z-10 flex flex-col h-full">
              <div className="mb-8">
                ...
                <p className="text-emerald-200 text-xs leading-relaxed font-medium">
                  所有站点自动配置 Cloudflare SSL/TLS 证书，集成 WAF
                  防火墙机制，确保全球访问速度与数据安全。
                </p>
              </div>
