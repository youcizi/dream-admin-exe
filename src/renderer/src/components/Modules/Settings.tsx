        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            <div className="bg-indigo-900 rounded-[2.5rem] p-8 text-white">
              ...
              <button className="mt-8 w-full py-4 bg-white/10 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-white/20 transition-all">
                刷新状态
              </button>
            </div>
          </div>
        ...
        <div className="max-w-4xl space-y-8">
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8">
                ...
                <div className="space-y-4 pt-6">
                  <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100 flex items-start gap-3">
                    ...
                    <p className="text-[10px] text-amber-700 font-medium leading-relaxed">
                      API 密钥将加密存储在本地 SQLite
                      数据库中。我们绝不会将密钥上传到任何非官方服务器。
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-12 flex justify-end gap-3 border-t border-slate-50 pt-8">
              <button className="px-6 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl text-xs font-bold hover:bg-slate-50 active:scale-95 transition-all">
                重置更改
              </button>
      ...
      {subModule === '通用设置' && (
        <div className="max-w-4xl space-y-6">
          {[
            ...
            <div
              key={i}
              className="flex items-center justify-between p-8 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm hover:border-indigo-100 transition-all group cursor-pointer"
            >
