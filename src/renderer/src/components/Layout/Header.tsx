    ...
    <header className="h-14 border-b border-slate-200/60 bg-white/80 backdrop-blur-md flex items-center justify-between px-6 z-50 relative">
      ...
      <div className="hidden md:flex items-center gap-6 px-4 py-1.5 bg-slate-50 rounded-full border border-slate-100">
        ...
        <div className="flex items-center gap-2 text-xs font-bold text-slate-500">
          ...
          <span>
            Balance: <span className="text-slate-700">$20.70</span>
          </span>
        </div>
        <div className="w-px h-3 bg-slate-200" />
        <div className="flex items-center gap-2 text-xs font-bold text-slate-500">
          ...
          <span>
            AI Status: <span className="text-emerald-500">Ready</span>
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          ...
          >
            ...
            <ChevronDown
              size={14}
              className={`transition-transform duration-300 ${activeMenu === 'settings' ? 'rotate-180' : ''}`}
            />
          </button>
        ...
        <div className="relative">
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 rounded-xl transition-all ${
              activeMenu === 'me' ? 'bg-indigo-50 text-primary' : 'text-slate-600 hover:bg-slate-50'
            }`}
          >
            ...
            <ChevronDown
              size={14}
              className={`transition-transform duration-300 ${activeMenu === 'me' ? 'rotate-180' : ''}`}
            />
          </button>
            ...
            <>
              ...
              <div className="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-2xl border border-slate-100 py-2 animate-in fade-in zoom-in duration-200 origin-top-right overflow-hidden">
                <div className="px-4 py-2 mb-1 border-b border-slate-50 flex items-center gap-3">
                  <div className="w-8 h-8 bg-indigo-100 rounded-full flex items-center justify-center text-primary font-bold text-xs">
                    Admin
                  </div>
                  <div>
