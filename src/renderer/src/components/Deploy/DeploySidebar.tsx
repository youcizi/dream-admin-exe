    ...
    <div className="w-72 bg-slate-900 flex flex-col h-full shadow-2xl z-40 relative">
      <div className="p-8 pb-4">
        ...
        <div className="space-y-1.5">
          <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest px-4 mb-3">
            核心管理
          </p>
          {menuItems.map((item) => (
            <button
            ...
            >
              <item.icon
                size={20}
                className={activeTab === item.id ? 'scale-110' : 'group-hover:scale-110'}
              />
              <div className="text-left">
                <p className="text-sm font-bold">{item.label}</p>
                <p
                  className={`text-[10px] ${activeTab === item.id ? 'text-indigo-100/70' : 'text-slate-500'}`}
                >
                  {item.desc}
                </p>
              </div>
